...
describe('repo preparation', () => {
  it('prepares execution branch for new and existing remote branches', async () => {
    ...
    expect(
      runChecked('git', ['branch', '--show-current'], { cwd: firstRun.repoPath }),
    ...
    writeFileSync(join(firstRun.repoPath, 'branch-file.txt'), 'branch change\n')
    runChecked('git', ['add', '--all'], { cwd: firstRun.repoPath })
    runChecked('git', ['commit', '-m', 'feat: create execution branch commit'], {
      cwd: firstRun.repoPath,
    })
    ...
    expect(
      runChecked('git', ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}'], {
...
function createSeededRemoteRepository(): SeededRepository {
  ...
  runChecked('git', ['commit', '-m', 'chore: initial commit'], { cwd: seedPath })
  runChecked('git', ['remote', 'add', 'origin', remotePath], { cwd: seedPath })
  runChecked('git', ['push', '--set-upstream', 'origin', 'main'], { cwd: seedPath })
