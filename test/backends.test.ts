...
describe("backend defaults and selection", () => {
  it("keeps internal as default backend", () => {
    expect(DEFAULT_BACKEND).toBe("internal");
  });

  it("creates adapters for all supported backend kinds", () => {
    const kinds: BackendKind[] = ["internal", "codex", "claude", "opencode"];
...
describe("internal backend adapter", () => {
  it("fails gracefully when required OpenRouter environment is missing", async () => {
    const repoPath = createTempDir("sidekick-backends-internal-missing-env-");
    const adapter = createBackendAdapter("internal");
  ...
  it("executes successfully in-process with mocked OpenRouter completion", async () => {
    const repoPath = createTempDir("sidekick-backends-internal-success-");
    const adapter = createBackendAdapter("internal");
                ...
                choices: [
                  {
                    ...
                    message: {
                      role: "assistant",
                      content: "Internal backend completed task.",
                      tool_calls: [],
                    },
                  },
                ],
    ...
    try {
      ...
      expect(result.output).toContain("Internal backend completed task.");
    } finally {
