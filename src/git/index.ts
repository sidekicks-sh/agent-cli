...
async function createPullRequest(input: {
  ...
  const prUrl = extractFirstUrl(createResult.stdout) ?? extractFirstUrl(createResult.stderr)
  return {
    outcome: 'success',
...
function indicatesExistingPullRequest(result: { stdout: string; stderr: string }) {
  const output = `${result.stdout}\n${result.stderr}`.toLowerCase()
  return output.includes('already exists') || output.includes('a pull request for branch')
}
