...
export class ControlPlaneClient {
  ...
  private readonly fetchImpl: NonNullable<ControlPlaneClientOptions['fetchImpl']>
  ...
  private async request(options: RequestOptions): Promise<unknown> {
    const url = new URL(trimLeadingSlash(options.path), this.baseUrl).toString()
...
function isRequestError(error: unknown) {
  return error instanceof Error && error.message.startsWith('Control plane request')
}
