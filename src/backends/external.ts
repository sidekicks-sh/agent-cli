...
export function createExternalBackendAdapter(
  kind: Exclude<BackendKind, 'custom'>,
  options: BackendFactoryOptions = {},
): BackendAdapter {
  const logBatchSize = parseLogBatchSize(options.logBatchSize)
  return {
    kind,
    runTask: async (input, context) => runExternalTask(kind, input, context, logBatchSize),
  }
}
...
async function runExternalTask(
  ...
  lineBatcher.push('internal', `starting backend command: ${invocation.command}`)
...
function asValidationFailure(kind: BackendKind, error: unknown): BackendTaskResult {
